...
import { Send, Bot, User, RefreshCw, BrainCircuit, MessageSquare, ArrowRight, ShieldCheck } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

const AiCompanion = () => {
  const [mode, setMode] = useState('chat'); // 'chat' | 'deconstruct'
    ...
    <div className="h-[calc(100vh-2rem)] md:h-screen flex flex-col p-4 md:p-8 max-w-5xl mx-auto">
      <header className="mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="bg-white p-1 rounded-xl border border-secondary/20 flex shadow-sm">
            ...
            >
                <BrainCircuit className="w-4 h-4" /> Thought Deconstruction
            </button>
        </div>
      </header>
                    ...
                    >
                        <div className="bg-red-50 p-6 rounded-2xl border border-red-100">
                            <h3 className="font-bold text-red-800 mb-2 flex items-center gap-2">
                                <AlertTriangleIcon className="w-5 h-5" /> Cognitive Distortions
                            </h3>
                            <ul className="list-disc list-inside text-red-700 space-y-1">
                                {deconstructionResult.distortions.map((d, i) => (
                                    <li key={i}>{d}</li>
                                ))}
                            </ul>
                        </div>
                        ...
                        <div className="bg-orange-50 p-6 rounded-2xl border border-orange-100">
                            <h3 className="font-bold text-orange-800 mb-2">Challenge the Thought</h3>
                            <p className="text-orange-900 leading-relaxed">{deconstructionResult.challenge}</p>
                        </div>

                        <div className="bg-green-50 p-6 rounded-2xl border border-green-100 shadow-sm">
                            <h3 className="font-bold text-green-800 mb-2 flex items-center gap-2">
                                <ShieldCheck className="w-5 h-5" /> Healthy Reframe
                            </h3>
